...
**Edit `better-language/`. Nothing else is source.**

`better-language.skill` is a build output. It isn't committed — CI builds it and attaches it to each release. So there's no archive to update in your pull request, and no way for your branch to conflict with someone else's on a binary.
...
**The frontmatter `name:` must match the folder name.** Both are `better-language`. The folder name becomes the skill's identity once installed, so if they drift, the skill installs wrong.
...
Add the pattern to the relevant table in `better-language/references/clutter.md`. If it's a structural tic rather than a word-level one, it likely belongs in the "Antithesis and Contrast Scaffolding" section of `better-language/SKILL.md` too.
...
The repo is a writing skill, so the prose in it should pass its own rules. Before submitting, read your addition against `better-language/references/checklist.md`. Watch for the antithesis constructions the skill exists to remove — they're easy to write by reflex while documenting them.
